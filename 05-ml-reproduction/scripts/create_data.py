...
# FULL_PATH = os.path.join(Path.cwd().parent, 'ntnu/AI-Driven (ML)/2024 ML/ML_Pipeline/data/raw/photocell_database.csv')
# print("Found datafile:", os.path.isfile(FULL_PATH))

project_root = Path(__file__).resolve().parent.parent

# 2. 直接用 / 符號拼接路徑
file_path = project_root / "data" / "raw" / "photocell_database.csv"

print(f"讀取的檔案路徑為: {file_path}")
print("Found datafile:", os.path.isfile(file_path))
FULL_PATH = file_path
# sys.exit()
...
def standard_scale_features(df: pd.DataFrame) -> pd.DataFrame:
  ...
  project_root = Path(__file__).resolve().parent.parent
  scaler_output_path = project_root / 'data' / 'cleaned' / 'scaler_info.csv'
  # scaler_output_path = os.path.join(Path.cwd().parent, 'data/cleaned/scaler_info.csv')
  scaler_info.to_csv(scaler_output_path, index=False)
  print(f"\t已成功儲存各物理參數的平均值與標準差至: {scaler_output_path}")
  # --------------------------------------------------
...
def main():
  ...
  project_root = Path(__file__).resolve().parent.parent
  output_path = project_root / 'data' / 'cleaned' / 'cleaned_data.csv'
